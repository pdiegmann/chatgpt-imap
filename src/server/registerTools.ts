...
export function registerTools(server: McpServer): void {
	...
	server.tool(
		...
		"Lists all mailbox folders and detects special folders (inbox, archive, drafts, sent, trash, junk). Does not modify any state.",
		{
			include_stats: z
				.boolean()
				.default(false)
				.describe("Include message_count and unseen_count per folder"),
	...
	server.tool(
		...
		async (input) => {
			try {
				...
				if (!email) {
					return toToolResult(
						makeError("EMAIL_NOT_FOUND", `Email not found: ${input.email_ref}`),
					);
				}
	...
	server.tool(
		...
		async (input) => {
			try {
				if (!input.destination_folder && !input.destination_special_use) {
					return toToolResult(
						makeError(
							"AMBIGUOUS_TARGET",
				...
				const result = await withImapClient(config, async (client) => {
					...
					return moveEmails(
						...
						ACCOUNT_ID,
						input.email_refs,
						// biome-ignore lint/style/noNonNullAssertion: this is safe because we check above that either destination_folder or destination_special_use is provided, and if it's special_use we resolve it to a path or return early if it can't be resolved
						destPath!,
	...
	server.tool(
		...
		async (input) => {
			try {
				...
				if (input.draft_type === "reply" && !input.reply_to_email_ref) {
					return toToolResult(
						makeError(
							"INVALID_QUERY",
				...
				return toToolResult(makeError("DRAFT_CREATE_FAILED", String(e)));
			}
		},
	);
}
